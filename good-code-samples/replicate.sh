    ...
    if [ -n "$_dist" ]; then
        ...
        case "$_dist" in
            ...
            centos)
                ...
                ;;
            amzn)
                _error_msg="$_error_msg\nHowever detected version $_version is not one of 2017.09, 2017.03, 2016.09, 2016.03, 2015.09, 2015.03, 2014.09, 2014.03."
...
_installDocker() {
    ...
    if { [ "$LSB_DIST" = "rhel" ] || [ "$LSB_DIST" = "ol" ] ; } && [[ "${1}" == *"17.06"* ]]; then
        if yum list installed "container-selinux" >/dev/null 2>&1; then
            ...
            # Install container-selinux from official source
...
pull_docker_images() {
    docker pull "quay.io/replicated/replicated:stable-2.16.0"
    docker pull "quay.io/replicated/replicated-ui:stable-2.16.0"
}
...
tag_docker_images() {
    ...
    docker tag "quay.io/replicated/replicated:stable-2.16.0" "quay.io/replicated/replicated:current" 2>/dev/null \
        || docker tag -f "quay.io/replicated/replicated:stable-2.16.0" "quay.io/replicated/replicated:current"
    docker tag "quay.io/replicated/replicated-ui:stable-2.16.0" "quay.io/replicated/replicated-ui:current" 2>/dev/null \
        || docker tag -f "quay.io/replicated/replicated-ui:stable-2.16.0" "quay.io/replicated/replicated-ui:current"
}
...
install_operator() {
    ...
    if [ "$AIRGAP" != "1" ]; then
        ...
        echo -e "${URLGET_CMD} https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.16.0${NC}"
        ${URLGET_CMD} "https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.16.0" > /tmp/operator_install.sh
    fi
